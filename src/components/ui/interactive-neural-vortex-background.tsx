...
/**
 * WebGL "neural vortex" background, retuned to the Dignita orange palette.
 * Renders only a <canvas> — meant to be placed as an absolute background
  ...
  React.useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // Dignita orange / amber palette
        color = vec3(0.96, 0.42, 0.12);
        color = mix(color, vec3(1.0, 0.72, 0.24), 0.35 + 0.16 * sin(2.0 * u_scroll_progress + 1.2));
        color += vec3(0.55, 0.16, 0.0) * sin(2.0 * u_scroll_progress + 1.5);
        color = color * noise;
    ...
    const render = () => {
      ...
      gl.uniform2f(
        uPointerPosition,
        pointer.current.x / (rect.width || 1),
        1 - pointer.current.y / (rect.height || 1)
      );
      gl.uniform1f(
        uScrollProgress,
